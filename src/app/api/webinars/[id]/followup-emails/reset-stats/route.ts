...
export async function POST(
  ...
  try {
    ...
    const sends = await prisma.followUpEmailSend.findMany({
    ...
    const result = await prisma.$transaction(async (tx) => {
      const deletedEvents = sendIds.length > 0
        ? await tx.emailTrackingEvent.deleteMany({
            where: { followUpEmailSendId: { in: sendIds } },
          })
        : { count: 0 }

      const resetSends = await tx.followUpEmailSend.updateMany({
        where: { templateId: { in: templateIds } },
        data: {
          openedAt: null,
          clickedAt: null,
    ...
    console.error('Failed to reset follow-up email stats:', error)
