/**
 * Registration Page Preview API
 * 
 * GET /api/registration-pages/[id]/preview
 * Returns rendered HTML of the registration page with sample data
 */
...
/**
 * GET /api/registration-pages/[id]/preview
 * Returns the registration page HTML with sample webinar data for preview
 */
export async function GET(
  ...
  try {
    ...
    if (!registrationPage) {
      return new NextResponse('Registration page not found', { status: 404 });
    }
    ...
    let html = registrationPage.htmlCode;
    ...
    const previewBanner = `
      ...
      ">
        🎨 REGISTRATION PAGE PREVIEW - Sample Data Shown
      </div>
    ...
    if (html.includes('<body')) {
      ...
      html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Registration Page Preview</title></head><body>${previewBanner}${html}</body></html>`;
    }
    ...
    console.error('Error generating registration page preview:', error);
