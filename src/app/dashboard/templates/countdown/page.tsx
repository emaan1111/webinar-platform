...
export default function CountdownTemplatesPage() {
  const router = useRouter()
  const [templates, setTemplates] = useState<CountdownTemplate[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedTemplate, setSelectedTemplate] = useState<CountdownTemplate | null>(null)
  ...
  const handleEdit = (template: CountdownTemplate) => {
...
<head>
    ...
    <style>
        ...
        .time-unit { display: inline-block; margin: 0 10px; }
        .btn {
...
<body>
    <div class="container">
        ...
        <p>Get ready! The webinar will begin shortly.</p>
        <a href="{{roomLink}}" class="btn">Enter Webinar Room</a>
    </div>
    ...
    <script>
        ...
        function updateCountdown() {
            ...
            if (distance < 0) {
                document.getElementById('countdown').innerHTML = 'Webinar is Live!';
                return;
            }
            ...
            let html = '';
            if (days > 0) html += '<span class="time-unit">' + days + 'd</span>';
            html += '<span class="time-unit">' + hours + 'h</span>';
            html += '<span class="time-unit">' + minutes + 'm</span>';
            html += '<span class="time-unit">' + seconds + 's</span>';
  ...
  const handleSave = async () => {
    try {
      const url = editingTemplate.id
        ? `/api/countdown-templates/${editingTemplate.id}`
        : '/api/countdown-templates'
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/countdown-templates/${id}`, {
        method: 'DELETE'
      })
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900">Countdown Page Templates</h1>
            <p className="text-gray-600 mt-2">
              Manage templates shown before webinars start
            </p>
          </div>
          <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Modern Countdown"
                  />
                </div>
