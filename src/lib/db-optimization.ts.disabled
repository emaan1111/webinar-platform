...
export async function getDashboardStatsOptimized(userId: string) {
  const [
    ...
    prisma.webinar.count({ where: { hostId: userId } }),
    prisma.registration.count({
      where: { webinar: { hostId: userId } },
    }),
    prisma.registration.count({
      where: { webinar: { hostId: userId }, attended: true },
    }),
  ])
  ...
  return {
    ...
    totalRegistrations,
    totalAttendees,
    upcomingWebinars: 0, // TODO: Calculate based on schedules
    attendanceRate:
